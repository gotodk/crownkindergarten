...
public class NoReSet_160922000059
{
    ...
    public DataSet NRS_ADD(DataTable parameter_forUI)
    {
        ...
        //遍历子表， 插入 
        string zibiao_gts_id = "grid-table-subtable-160922000235";
        ...
        for (int i = 0; i < subdt.Rows.Count; i++)
        {
            ...
            param.Add("@sub_" + "r_cpbh" + "_" + i, subdt.Rows[i]["出库品号"].ToString());
            param.Add("@sub_" + "r_shuliang" + "_" + i, subdt.Rows[i]["出库数量"].ToString());
            ...
            ArrayList al_kc = CKC.get_sql_str(chuku_kw, ruku_kw, subdt.Rows[i]["出库品号"].ToString(), subdt.Rows[i]["批号"].ToString(), subdt.Rows[i]["出库数量"].ToString(), subdt.Rows[i]["单位"].ToString(), "标准");
        ...
        if ((bool)(return_ht["return_float"]))
        {
            ...
            dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "提交成功！相应库存已变更！{"+ guid + "}";
        }
        else
        {
            dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
