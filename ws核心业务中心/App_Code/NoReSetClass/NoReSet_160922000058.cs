...
public class NoReSet_160922000058
{
    ...
    public DataSet NRS_ADD(DataTable parameter_forUI)
    {
        ...
        //遍历子表， 插入 
        string zibiao_gts_id = "grid-table-subtable-160922000224";
        ...
        for (int i = 0; i < subdt.Rows.Count; i++)
        {
            ...
            ArrayList al_kc = CKC.get_sql_str(chuku_kw, ruku_kw, subdt.Rows[i]["调整品号"].ToString(), subdt.Rows[i]["批号"].ToString(), subdt.Rows[i]["调整数量"].ToString(), subdt.Rows[i]["单位"].ToString(), "标准");
